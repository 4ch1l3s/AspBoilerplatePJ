...
using internPJ3.Models.TokenAuth;
...
{
    public abstract class internPJ3WebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static internPJ3WebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(internPJ3WebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<internPJ3DbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<internPJ3DbContext, Task> action)
        {
            using (var context = IocManager.Resolve<internPJ3DbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<internPJ3DbContext, Task<T>> func)
        {
            T result;
